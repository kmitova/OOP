...
class WorkerTests(TestCase):
    def test_worker_initialised_correctly(self):
    ...
    def test_worker_is_paid_after_work(self):
        worker = Worker("Test", 10000, 50)
        self.assertEqual(0, worker.money)
        worker.work()
    ...
    def test_energy_decreases_after_work(self):
        worker = Worker("Test", 10000, 50)
        self.assertEqual(50, worker.energy)
        worker.work()
        self.assertEqual(49, worker.energy)

    def test_get_info_return_correctly(self):
        worker = Worker("Test", 10000, 50)
