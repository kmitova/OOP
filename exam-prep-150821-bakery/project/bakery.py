...
class Bakery:
    ...
    def order_food(self, table_number, *args):
        ...
        for food_name in args:
            ...
            if food is None:
                ...
                ordered += str(food) + '\n'
        return ordered + not_in_menu.strip()
    ...
    def order_drink(self, table_number, *args):
        ...
        ordered_d = f"Table {table_number} ordered:\n"
        not_in_menu_d = f"{self.name} does not have in the menu:\n"

        for drink_name in args:
            drink = self.find_drink_by_name(drink_name)
            if drink is None:
                not_in_menu_d += drink_name + '\n'
            else:
                table.order_drink(drink)
                ordered_d += str(drink) + '\n'
        return ordered_d + not_in_menu_d.strip()
